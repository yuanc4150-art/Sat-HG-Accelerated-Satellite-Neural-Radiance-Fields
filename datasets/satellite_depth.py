...
class SatelliteDataset_depth(SatelliteDataset):
    def __init__(self, root_dir, img_dir, split="train", img_downscale=1.0, cache_dir=None):
        ...
        """
        super().__init__(root_dir, img_dir, split=split, img_downscale=img_downscale, cache_dir=cache_dir)
    ...
    def load_depth_data(self, json_files, tie_points, verbose=False):
        ...
        for t, json_p in enumerate(json_files):
            ...
            pts3d_ecef = np.array(tie_points[d["keypoints"]["pts3d_indices"], :])
            ...
            center_lon, center_lat = rpc.localization(np.array([cols.mean()]), np.array([rows.mean()]),
                                                      np.array([(min_alt + max_alt) / 2]))


            pts3d_x_utm, pts3d_y_utm = sat_utils.ecef_to_utm_custom(pts3d_ecef[:, 0], pts3d_ecef[:, 1],
                                                                    pts3d_ecef[:, 2],
                                                                    center_lat[0], center_lon[0])


            _, _, alts_3d = sat_utils.ecef_to_latlon_custom(pts3d_ecef[:, 0], pts3d_ecef[:, 1], pts3d_ecef[:, 2])


            pts3d_utm = np.vstack([pts3d_x_utm, pts3d_y_utm, alts_3d]).T
            ...
            pts3d = torch.from_numpy(pts3d_utm).float()
        ...
        all_depths = torch.hstack([all_depths, all_weights])
        all_sun_dirs = torch.cat(all_sun_dirs, 0)
        all_rays = torch.hstack([all_rays, all_sun_dirs])
